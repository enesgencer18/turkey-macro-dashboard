...
Mathematically, you optimize the following equation:
...
I use time-series cross-validation with a fixed window size to tune the hyper-parameters and evaluate the model performance. The idea is that the inflation dynamics might evolve. For instance, we have observed the increasing effect of the exchange rate on inflation in recent years. However, as a result of the commodity super-cycle, we might see the producer price index come into play. To learn more details on time-series cross-validation, please visit: https://robjhyndman.com/hyndsight/tscv/
